...
@endif
<section>
    <form id="form-brand" enctype="multipart/form-data" method="post" action="{{route('admin.brand.update', ['id' => $brand->id])}}">
        @csrf
        @method('put')
    </form>
</section>
<section class="w-full">
    <div>
        <div class="relative flex justify-center overflow-hidden items-center aspect-square w-40 border-2 border-dashed border-color-3 rounded">
            ...
            <img src="{{asset('storage/'.$brand->image)}}" alt="" class="absolute w-40 -z-10">
        </div>
    </div>
</section>
...
<section class="grid-cols-1 lg:grid-cols-2 grid w-full lg:w-8/12 gap-3">
    <div>
        ...
        <input required form="form-brand" name="name" type="text" value="{{ old('name', $brand->name) }}" class="w-full p-2 rounded border border-color-3 focus:outline-none">
    </div>
</section>
<button form="form-brand" type="submit" class="h-[42px] w-full sm:w-fit rounded text-white bg-color-5 p-2">Simpan</button>
@push('script')
    <script>
