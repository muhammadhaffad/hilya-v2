...
@section('content')
    ...
    <form id="form-place-order" action="{{ route('customer.checkout.place-order') }}" method="post">
        @csrf
    </form>
    ...
    @foreach ($checkoutItems->orderItems as $checkoutItem)
        <div class="{{ $checkoutItem->productItem->stock < $checkoutItem->qty ? 'bg-color-1' : '' }} flex flex-wrap items-center justify-between gap-2 p-2 border border-color-4 rounded-lg">
            ...
            <div class="max-w-[160px]">
                <ul>
                    <li class="font-bold">{{ $checkoutItem->productItem->color }}</li>
                    <li>{{ $checkoutItem->productItem->note_bene }}</li>
                </ul>
            </div>
            <div class="w-max">
                <ul>
    ...
    <div class="w-full space-y-4">
        ...
        @foreach ($shippingCost as $key => $cost)
            <div class="flex flex-wrap">
                <div class="w-full sm:w-1/2 uppercase mb-2">
                    ...
                    <label for="kurir-{{$key}}" class="block py-2 px-4 w-max uppercase font-medium border border-color-4 text-color-4 rounded cursor-pointer peer-checked:bg-color-1 peer-checked:text-blue-600 peer-checked:border-blue-600">
                        <div>
                            {{ $cost['courier'] }}
                        </div>
                    </label>
    ...
    <div class="flex justify-end gap-2">
        ...
        @csrf
            <button class="py-2 px-4 uppercase font-medium border border-color-4 rounded">
                Kembali ke Keranjang
            </button>
        </form>
        <button type="submit" class="py-2 px-4 uppercase font-medium border bg-color-4 border-color-4 text-white rounded" form="form-place-order">
            Buat Pesanan
        </button>
    </div>
    @push('script')
